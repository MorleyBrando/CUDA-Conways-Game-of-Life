...
__global__ void compute_on_gpu(int *life, int *previous_life, int X_limit, int Y_limit) {
	...
	int src_X = threadIdx.x + blockIdx.x * blockDim.x;
        int src_Y = threadIdx.y + blockIdx.y * blockDim.y;
	...
	int neighbors = 0;
        ...
        for(int i = src_X; i < X_limit; i += stride_X){
               for (int j = src_Y; j < Y_limit; j += stride_Y){
			neighbors = previous_life[(i) * (Y_limit + 1) + j + 1] + previous_life[(i) * (Y_limit + 1) + j + 2] +
			       previous_life[(i) * (Y_limit+ 1) + j + 3]	+ previous_life[(i + 1) * (Y_limit + 1) + j + 1] + 
			       previous_life[(i + 1) * (Y_limit + 1) + j + 3] + previous_life[(i + 2) * (Y_limit + 1) + j + 1] + 
			       previous_life[(i + 2) * (Y_limit + 1) + j+ 2] + previous_life[(i + 2) * (Y_limit + 1) + j + 3];

			if (previous_life[(i + 1) * (Y_limit + 1) + j + 2] == 0){
				if(neighbors == 3){
...
void copy_grid_to_device(int *host_array, int *device_array, int length) {
    /* your code here */
    cudaError_t status = cudaMemcpy(device_array, host_array, sizeof(int)*length, cudaMemcpyHostToDevice);
    if(status != cudaSuccess) {
	//cderr<< "Could not copy to device Error code:" << status << "\n";
    	cout << "Error grid to device" << status << "\n";
    }
}
...
void copy_grid_to_host(int *host_array, int *device_array, int length) {
    /* your code here */
    cudaError_t status = cudaMemcpy(host_array, device_array, sizeof(int)*length, cudaMemcpyDeviceToHost);
    if(status != cudaSuccess) {
        //cderr << "Could not copy to host Error code:" << status << "\n";
    	cout << "Error grid to host" << status << "\n";
    }
}
...
__global__ void padded_matrix_copy(int *dst, int *src, int width, int height, int padding) {
	...
	for(int i = src_X; i < height; i += stride_X){
	       for (int j = src_Y; j < width; j += stride_Y){

		dst[(i + padding) * (width + padding) + j + 1  + padding] = src[i * width + j];      
